...
public class GrafoEtiq<T> {
	...
	private Lista anchuraDesde(NodoVert vertInicial, Lista visitados) {
		Lista l = new Lista();
		l.insertar(vertInicial, l.getLongitud() + 1);
		NodoVert u;
		NodoAdy arco;
		while (l.getLongitud() != 0) {
			u = (NodoVert) l.recuperar(1);
			l.eliminar(1);
			visitados.insertar(u.getElem(), visitados.getLongitud() + 1);
			arco = u.getPrimerAdy();
			while (arco != null) {
				if (l.localizar(arco.getVertice()) < 0 && visitados.localizar(arco.getVertice().getElem()) < 0) {
					l.insertar(arco.getVertice(), l.getLongitud() + 1);
				}
	...
	private Lista caminoMasCortoAux(NodoVert n, Object destino, Lista visitados, double distActual, Lista camino,
			double[] distMin) {
		if (n != null) {
			visitados.insertar(n.getElem(), visitados.getLongitud() + 1);
			...
			visitados.eliminar(visitados.getLongitud());
		}
		return camino;
	}
	...
	private Lista caminoMenosNodosAux(NodoVert n, Object destino, Lista visitados, Lista camino) {
		...
		if (n != null) {
			...
			if (!n.getElem().equals(destino)) {
				...
				while (arco != null) {
					if (visitados.localizar(arco.getVertice().getElem()) < 0) {
						if (camino.getLongitud() != 0) {
							if (visitados.getLongitud() < camino.getLongitud()) {
				...
				if (camino.getLongitud() == 0 || visitados.getLongitud() < camino.getLongitud()) {
					camino = visitados.clone();
				}
			}
			visitados.eliminar(visitados.getLongitud());
